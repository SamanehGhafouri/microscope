...
Meteor.methods({
    ...
    upvote: function (postId) {
        // check the user logged in and that post really exist
        check(this.userId, String);
        // user has already voted for the post
        check(postId, String);

        //Mongo allows us to combine from grabing posts and 3 steps into one for vote
        //Better upvoting algorithm
        let affected = Posts.update({
            //find all the posts by id that this user hasn't voted for
            _id: postId,
        ...
        Posts.update(post._id, {
            // $addToset and $inc are Mongo operators.
            //adds an item to an array as long as it doesn't exist
            $addToSet: {upvoters: this.userId},
